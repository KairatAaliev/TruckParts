...
import com.example.truckparts.stock_fragment.adapter.StockNewsAdapter
import com.example.truckparts.then_delete.StockModel

class StockNewsRVFragment : Fragment() {

    lateinit var recyclerView: RecyclerView
    private var adapter: StockNewsAdapter? = null

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        return inflater.inflate(R.layout.fragment_stock_news_r_v, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        adapter = StockNewsAdapter()
        recyclerView = requireView().findViewById(R.id.recycler_view_stock_news)
        recyclerView.layoutManager = LinearLayoutManager(requireContext())
        recyclerView.adapter = adapter
        adapter?.setData(getNewsList())
    }

    private fun getNewsList(): List<StockModel> {
